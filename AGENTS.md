...
- **Lint policy is copied from upstream, like the source is.** [eslint.config.js](eslint.config.js) mirrors canvas-editor's own — same presets (`js.configs.recommended` + `tseslint.configs.recommended`), same rule exemptions — so it stays diffable against theirs. Two local additions are marked in the file: this repo's extra `ignores`, and `no-unused-vars` with `argsIgnorePattern: '^_'`, which covers parameters kept for signature parity with upstream but unused here. Resist reaching for `tseslint.configs.recommendedTypeChecked`: it lights up ~190 findings, almost all of them in verbatim copies of upstream code, and "fixing" those is what makes the next port stop applying cleanly.
